...
function EventPreview({ event }: EventPreviewProps) {
  ...
  return (
    <Link
      href={`/dashboard/planning/events/${event.eventid}`}
...
function UnitPreview({ unitId, unitName, memberCount, eventCount }: UnitPreviewProps) {
  return (
    <Link
      href={`/dashboard/planning/events/units/${encodeURIComponent(unitId)}`}
      className="flex items-center justify-between p-3 rounded-lg hover:bg-muted/50 transition-colors group"
    >
      <div className="min-w-0 flex-1">
...
export function PlanningDashboardHome() {
  return (
    <div className="p-4 md:p-6">
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-bold">Expedition Planner</h1>
        <p className="text-muted-foreground mt-1">
          Plan and manage events and logistics for your section
        </p>
      </div>
